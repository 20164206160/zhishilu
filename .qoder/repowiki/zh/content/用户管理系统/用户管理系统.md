...
用户服务层负责处理用户相关的业务逻辑，包括注册、登录、密码处理等核心功能。**更新** 增加管理员用户管理功能。
...
**章节来源**
- [UserService.java](file://src/main/java/com/zhishilu/service/UserService.java#L25-L231)

## 管理员功能模块

系统新增管理员功能模块，提供完整的用户管理能力。**新增** 管理员可以管理所有用户，包括查看用户列表、授权状态管理、用户删除等操作。
...
4. **取消授权**：将authorized字段设置为0，并强制用户登出
5. **删除用户**：删除用户并强制登出

**章节来源**
- [UserController.java](file://src/main/java/com/zhishilu/controller/UserController.java#L115-L222)
- [AdminConfig.java](file://src/main/java/com/zhishilu/config/AdminConfig.java#L15-L47)

## API接口规范

系统提供RESTful API接口，遵循统一的响应格式和错误处理机制。**更新** 新增管理员用户管理相关接口。
...
系统采用多层次的安全策略，确保用户数据和系统资源的安全性。**更新** 增强管理员权限控制和用户授权机制。
...
系统采用JWT令牌实现无状态的会话管理，避免了传统Session的服务器端存储开销。**更新** 增强会话管理，支持管理员强制用户登出。
...
**图表来源**
- [UserService.java](file://src/main/java/com/zhishilu/service/UserService.java#L209-L229)
...
系统采用全局异常处理器统一处理各种异常情况，提供一致的错误响应格式。**更新** 增加管理员权限相关的错误处理。
