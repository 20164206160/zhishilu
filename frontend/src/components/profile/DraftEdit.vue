<template>
  <div class="h-full flex flex-col overflow-hidden">
    ...
    <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
      ...
      <h1 class="text-base sm:text-lg font-black text-gray-900">编辑草稿</h1>
...
const getFormContentHash = () => JSON.stringify({
  title: form.title,
...
onMounted(async () => {
  ...
  form.title = props.draft.title || '';
  form.categories = props.draft.categories || [];
  form.images = props.draft.images || [];
  form.content = props.draft.content || '';
  form.url = props.draft.url || '';
  form.location = props.draft.location || '';
...
const saveDraftSync = async () => {
  ...
  try {
    const draftData = { id: props.draft.id, ...form };
...
const autoSaveDraft = async () => {
  ...
  try {
    ...
    if (res.data.code === 200) {
      ...
      setTimeout(() => { if (autoSaveStatus.value === '已保存') autoSaveStatus.value = ''; }, 2000);
    }
  } catch (err) {
...
const startAutoSave = () => {
  ...
  autoSaveTimer.value = window.setInterval(autoSaveDraft, 10000);
};
...
const handlePublish = async () => {
  ...
  loading.value = true;
  try {
    const res = await request.post(`/article/draft/${props.draft.id}/publish`, form);
    if (res.data.code === 200) {
      alert('发布成功！');
