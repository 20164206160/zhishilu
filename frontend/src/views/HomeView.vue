                ...
                <div class="flex-grow overflow-y-auto">
                  ...
                  <div class="border-t border-[#f0f0f0]">
                    ...
                    <div class="px-5 pb-4 space-y-4">
                      <!-- 加载中 -->
                      <div v-if="commentLoading && comments.length === 0" class="py-5 text-center text-[13px] text-[#999]">
                      ...
                      <!-- 暂无评论 -->
                      <div v-else-if="!commentLoading && comments.length === 0" class="py-5 text-center text-[13px] text-[#bbb]">
                        暂无评论，来发表第一条吧~
                      </div>
                      ...
                      <div v-for="comment in comments" :key="comment.id" class="flex gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <p class="text-[14px] text-[#333] mt-0.5 leading-relaxed break-words">{{ comment.content }}</p>
                          ...
                          <!-- 回复列表 -->
                          <div v-if="getDisplayReplies(comment).length" class="mt-3 bg-[#f8f8f8] rounded-lg p-3 space-y-2.5">
                              ...
                              <div class="flex items-start gap-2">
                                ...
                                <div class="flex-1 min-w-0">
                                  <div class="flex flex-wrap items-baseline gap-1">
                                    ...
                                    <span class="text-[#aaa] text-[11px]">{{ formatModalDate(reply.createdTime) }}</span>
                                  </div>
                            ...
                            <div v-if="comment.totalReplyCount > 3" class="mt-1">
                              <button v-if="!expandedReplies.has(comment.id)" @click="expandReplies(comment.id)" class="text-[12px] text-[#576b95] hover:underline">
                                查看全部 {{ comment.totalReplyCount }} 条回复
                              </button>
                              <button v-else @click="collapseReplies(comment.id)" class="text-[12px] text-[#999] hover:underline">收起回复</button>
                            </div>
                          </div>
                        </div>
                        <button @click="handleLikeComment(comment)" class="flex flex-col items-center gap-0.5 text-[#999] hover:text-blue-600 transition-colors flex-shrink-0">
                          <Heart :size="14" />
                      ...
                      <!-- 加载更多 -->
                      <div v-if="hasMoreComments" class="pt-1 text-center">
                        <button @click="loadMoreComments" :disabled="commentLoading" class="text-[13px] text-[#576b95] hover:underline disabled:opacity-50">
                  ...
                  <div class="relative">
                    ...
                    <div class="relative flex items-center">
                      ...
                      <input
                        ...
                        class="w-full bg-[#f5f5f5] border-none rounded-full py-2.5 pl-9 pr-14 text-[14px] text-[#333] placeholder:text-[#999] focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all"
                      />
                      <button @click="sendComment" class="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1 text-blue-600 text-[13px] font-medium hover:bg-blue-50 rounded-full transition-colors">
                        发送
                      </button>
                    </div>
                    ...
                    <teleport to="body">
                      <div
                        v-if="showEmojiPanel"
                        @click.stop
                        class="fixed z-[300] bg-white border border-gray-200 rounded-2xl shadow-xl p-3"
                    ...
                    <div v-if="showEmojiPanel" @click="showEmojiPanel = false" class="fixed inset-0 z-[299]"></div>
                  </div>
                </div>
              </aside>
            </div>
...
// 模拟评论数据
// const mockComments = ... 已移除，改为真实 API
...
// 加载评论
const loadComments = async (articleId: string, reset = false) => {
  if (commentLoading.value) return;
  commentLoading.value = true;
  try {
    const pg = reset ? 0 : commentPage.value;
    const res = await request.get('/comment/list', {
      params: { articleId, page: pg, size: commentPageSize }
    });
    if (res.data.code === 200) {
...
const loadMoreComments = async () => {
  ...
  commentPage.value++;
  await loadComments(modalArticle.value.id, false);
};
...
const insertEmoji = (emoji: string) => {
  ...
  if (!input) { commentInput.value += emoji; return; }
...
const sendComment = async () => {
  ...
  if (!content || !modalArticle.value) return;
  const storedUser = localStorage.getItem('user');
  if (!storedUser) { alert('请先登录后再评论'); router.push('/login'); return; }
  try {
    const payload: any = { articleId: modalArticle.value.id, content };
    ...
    if (res.data.code === 200) {
      ...
      replyTarget.value = null;
      showEmojiPanel.value = false;
      await loadComments(modalArticle.value.id, true);
    }
  } catch (e: any) {
...
const handleDeleteComment = async (commentId: string) => {
  ...
  try {
    ...
    if (res.data.code === 200 && modalArticle.value) {
      await loadComments(modalArticle.value.id, true);
    }
  } catch (e: any) {
...
const handleLikeComment = async (comment: any) => {
  const storedUser = localStorage.getItem('user');
  if (!storedUser) { alert('请先登录'); return; }
  try {
...
const canDeleteComment = (comment: any) => {
  ...
  return user.username === comment.createdBy || user.admin === true;
};
